...
class UsersDB extends EventEmitter {
  ...
  // update() is a put() that uses locks to (try to) ensure atomicity
  // - use this instead of put() whenever possible to avoid data loss
  async update (id, updates) {
    assert(typeof id === 'string')
    assert(updates && typeof updates === 'object')
    var release = await lock('users:write:' + id)
    try {
      var record = await this.getByID(id)
      for (var k in updates) {
        if (k === 'id') continue // dont allow that!
        record[k] = updates[k]
      }
      record.updatedAt = Date.now()
      await this.accountsDB.put(record.id, record)
