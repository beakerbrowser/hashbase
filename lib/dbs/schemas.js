      ...
      async (user, next) => {
        if (user && user.archives && user.archives.length) {
          for (let archive of user.archives) {
            ...
            if (!archiveRecord) {
              console.error('No archive record found for a user\'s archive, removing...', {key: archive.key, user})
              user.archives = user.archives.filter(a => a.key !== archive.key)
              await this.cloud.usersDB.put(user)
              continue
            }
