...
module.exports = class Archiver extends EventEmitter {
  ...
  async _recomputeArchiveLatestStats (archive) {
    ...
    if (!metadata || !content || archive.version === 0) {
      return
    }
    ...
    // list all files
    var names = await pda.readdir(archive, '/', {recursive: true})

    // get stats on each file and update state
    await Promise.all(names.map(async name => {
      let fileSt = await pda.stat(archive, name)
      st.downloaded += fileSt.downloaded
      st.length += fileSt.blocks
      st.byteLength += fileSt.size
      st.files++
    }))
  }
