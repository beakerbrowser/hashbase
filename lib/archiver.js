...
module.exports = class Archiver extends EventEmitter {
  ...
  async _loadArchiveInner (archivePath, key) {
    ...
    else archive.on('content', onContentReady)
  ...
  async _computeArchiveLatestStats (archive) {
    ...
    try {
      ...
      var names = await pda.readdir(co, '/', {recursive: true})
      for (let name of names) {
        let fileSt = await pda.stat(co, name)
        st.numDownloadedBlocks += fileSt.downloaded
