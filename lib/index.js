...
class Hypercloud {
  ...
  async setup () {
    ...
    if (!db && dir) {
      // allocate a sqlite
      db = await sqlite.open(path.join(dir, 'main.db'))
    }
    ...
    await schemas.runCorrections()

    // final init
    await this.usersDB.setup()
    await this.archivesDB.setup()
    await this.activityDB.setup()
    await this.reportsDB.setup()
  ...
  async loadAllArchives () {
    // load all archives
    var ps = []
    var records = await this.archivesDB.listKeys()
    for (let key of records) {
      ps.push(this.archiver.loadArchive(records.key).then(null, _ => null))
  ...
  async setupAdminUser () {
    try {
      ...
      if (!isPassword(adminConfig.password)) {
        throw new Error('Admin user not created: invalid password')
      }
