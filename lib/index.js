...
class Hypercloud {
  constructor (config) {
    ...
    this.hyperdrive = hyperdrive(this.archiver.db)
    this.dat = ArchiverServer(this.archiver, swarmDefaults({
      swarm: true,
      http: false,
  ...
  async getArchive (key) {
    var archive = this.archiveCache.get(key)
    if (archive) return archive

    // look up the feeds from the archive
    var keyBuffer = Buffer.from(key, 'hex')
    var {metadata, content} = await new Promise((resolve, reject) => {
      this.archiver.get(keyBuffer, (err, metadata, content) => {
        if (err) reject(err)
        else resolve({metadata, content})
      })
    })
    if (!content) throw new Error('Not an archive')

    // create the archive
    archive = this.hyperdrive.createArchive(keyBuffer, {metadata, content})
    this.archiveCache.set(key, archive)
    return archive
  }
}
