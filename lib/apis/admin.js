...
module.exports = class AdminAPI {
 ...
 async listUsers (req, res) {
  ...
  async getReports (req, res) {
    ...
    // fetch
    var reports = await this.reportsDB.list({
      cursor: req.query.cursor,
      limit: req.query.limit ? +req.query.limit : 25,
      sort: req.query.sort,
      reverse: +req.query.reverse === 1
  ...
  async updateReport (req, res) {
    ...
    // validate & sanitize input TODO
    // validate fields that can be changed: status, notes
    // ;(await req.getValidationResult()).throw()
  ...
  async _getReport (id) {
    return await this.reportsDB.getByID(id)
