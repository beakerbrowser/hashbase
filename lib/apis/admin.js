...
module.exports = class AdminAPI {
  ...
  async getAnalyticsVisitsList (req, res) {
    // check perms
    if (!res.locals.session) throw new UnauthorizedError()
    if (!res.locals.session.scopes.includes('admin:dats')) throw new ForbiddenError()

    // run query
    var visits = await this.analytics.listVisits({
  ...
  async getAnalyticsVisitsCount (req, res) {
    // check perms
    if (!res.locals.session) throw new UnauthorizedError()
    if (!res.locals.session.scopes.includes('admin:dats')) throw new ForbiddenError()

    // run query
    var visits = await this.analytics.countEvents({
      unique: req.query.unique,
