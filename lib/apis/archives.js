...
module.exports = class ArchivesAPI {
  ...
  async archiveStatus (req, res) {
    ...
    if (type === 'text/event-stream') {
      ...
      await this._getArchive(req.params.key) // will throw if the archive is not actively
  ...
  _getArchiveProgressEventStream (req, res) {
    ...
    let self = this
    let lastProgress
    let nextSendDelay = 4 // in seconds, ramped up as changes dont occur
    ...
    async function send () {
      ...
      // ramp the delay
      if (typeof lastProgress === 'undefined' || lastProgress === progress) {
        if (nextSendDelay < 32) {
          nextSendDelay *= 2
        }
      } else {
      ...
      to = setTimeout(send, nextSendDelay * 1e3)
    }
    res.once('close', () => {
      this.activeProgressStreams--
      done = true
