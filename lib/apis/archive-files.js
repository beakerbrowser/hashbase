...
module.exports = class ArchiveFilesAPI {
  ...
  async _getArchiveRecord (vhost) {
    var username, archname
    if (!vhost[1]) {
      username = archname = vhost[0]
    } else {
      archname = vhost[0]
      username = vhost[1]
    }
    ...
    const findFn = a => a.name.toLowerCase() === archname
    var archiveRecord = userRecord.archives.find(findFn)
  ...
  async getDNSFile (req, res) {
    ...
    var archiveRecord = await this._getArchiveRecord(req.vhost)
  ...
  async getFile (req, res) {
    ...
    if (!entry) {
      ...
      return directoryListingPage(archive, req.url, html => res.end(html))
    }
