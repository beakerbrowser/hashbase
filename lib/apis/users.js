...
const {randomBytes, hashPassword, verifyPassword, shasum} = require('../crypto')
...
module.exports = class UsersAPI {
  constructor (cloud) {
    ...
    this.stripe = cloud.config.stripe ? Stripe(cloud.config.stripe.secretKey) : null
  }
  ...
  async doLogin (req, res) {
    ...
    try {
      ...
      userRecord = await this.usersDB.getByUsername(username)
      console.log('DEBUG login data', {userRecord, username, password, hash: shasum(Buffer.concat([
    Buffer.from(userRecord.passwordSalt, 'hex'),
    Buffer.from(password, 'utf8')
  ]))})
      assert(userRecord.isEmailVerified)
      assert(verifyPassword, shasum(password, userRecord))
    } catch (e) {
      return res.status(422).json({
