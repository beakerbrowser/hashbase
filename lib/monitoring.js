...
function setupArchiverMonitor (config, cloud, pmx) {
  ...
  var connErrsCounter = pmx.probe().counter({name: 'Connection Errors'})
  var connsMeter = pmx.probe().meter({name: 'Connections/s', samples: 1})
  cloud.archiver.on('new-connection', () => {
    connsCounter.inc()
    connsMeter.mark()
  })
  cloud.archiver.on('connection-errored', () => connErrsCounter.inc())
  cloud.archiver.on('connection-closed', () => connsCounter.dec())
}
